...
public class CarMovement : MonoBehaviour
{
    ...
    private const float TURN_SPEED = 150;
    ...
    public double horizontalInput { get; set; }
    public double verticalInput { get; set; } //Horizontal = turning force, Vertical = engine force
    /// <summary>
    ...
    {
        double velocidad = 0.5d;
        horizontalInput = 0.5d;
        if (direction == (int)CarController.Directions.derecha)
        {
            Rotation = transform.rotation;
            Rotation *= Quaternion.AngleAxis((float)horizontalInput * 500 * Time.deltaTime, new Vector3(0, 0, 1));
            
            this.SetInputs(new double[] { horizontalInput, velocidad});

        } else if(direction == (int)CarController.Directions.izquierda)
        {
            Rotation = transform.rotation;
            Rotation *= Quaternion.AngleAxis((float)-horizontalInput * 500 * Time.deltaTime, new Vector3(0, 0, 1));
            this.SetInputs(new double[] { -horizontalInput, velocidad });
